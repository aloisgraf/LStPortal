...
// Die Historie wird pro Kalendertag (5 Spalten, day_key OHNE Schichtart-Suffix,
// z.B. "24.12") in EINER Zelle mit 4 möglichen Werten erfasst; die Schichtart
// steckt im Zellwert selbst, nicht mehr in der Spalte:
//   TD (Tagdienst gearbeitet)  → +1 × Gewichtung auf die Tag-Dimension   ("<day>-T")
//   ND (Nachtdienst gearbeitet)→ +1 × Gewichtung auf die Nacht-Dimension ("<day>-N")
//   UR (Urlaub)                → −1 × Gewichtung auf BEIDE Dimensionen ("<day>-T" UND
//                                 "<day>-N") — UR sagt nicht aus, welche Schichtart
//                                 sonst gefragt gewesen wäre; 0/0 würde UR aber
//                                 funktional nicht von "–" unterscheidbar machen,
//                                 daher bewusst −1 auf beide statt neutral.
...
// Rückgabe bleibt unverändert im 10-Dimensionen-Format ({employeeId:{"<day>-T":score,...}}),
// damit buildChristmasProposal/UI (Score, Wunsch-Sortierung) unverändert bleiben.
function computeChristmasScores(historyRows) {
  const scores = {}; // employeeId -> {dayKey(-T/-N): score}
  const add = (empId, dayKey, delta) => {
    if (!scores[empId]) scores[empId] = {};
    scores[empId][dayKey] = (scores[empId][dayKey] || 0) + delta;
  };
  for (const h of historyRows) {
    if (!h.status) continue;
    if (h.status === 'TD') add(h.employee_id, `${h.day_key}-T`, 1 * christmasScoreWeight(`${h.day_key}-T`));
    else if (h.status === 'ND') add(h.employee_id, `${h.day_key}-N`, 1 * christmasScoreWeight(`${h.day_key}-N`));
    else if (h.status === 'UR') {
      add(h.employee_id, `${h.day_key}-T`, -1 * christmasScoreWeight(`${h.day_key}-T`));
      add(h.employee_id, `${h.day_key}-N`, -1 * christmasScoreWeight(`${h.day_key}-N`));
    }
    // '–' → 0, keine Dimension betroffen
  }
  return scores;
}
...
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  // historyRows liegen im 5-Spalten-Format (day_key ohne -T/-N-Suffix, Status
  // TD/ND kodiert die Schichtart) — für den Vorjahresvergleich auf die
  // 10-Key-Domäne (mit Suffix) abbilden, die applyAntiRepeatSwaps erwartet.
  const lastYearWorkedSet = new Set(
    (historyRows||[])
      .filter(h => h.year === year - 1 && (h.status === 'TD' || h.status === 'ND'))
      .map(h => `${h.employee_id}|${h.day_key}-${h.status === 'TD' ? 'T' : 'N'}`)
  );
...
module.exports = {
  ...
  isUserActive,
  // Weihnachtsdienst-Rotation
  CHRISTMAS_DAYS, CHRISTMAS_DAY_KEYS, CHRISTMAS_CALENDAR_DAY_KEYS, christmasDateFor, computeChristmasScores, buildChristmasProposal,
