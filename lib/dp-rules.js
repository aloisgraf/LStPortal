...
// ── ZENTRALE AKTIV-ABLEITUNG (Mitarbeiter) ────────────────────────────────────
// Kein gespeichertes Aktiv/Inaktiv-Flag — wird IMMER live aus hire_date/
// termination_date berechnet, damit sie im ganzen Portal einheitlich ist
// (db.js re-exportiert diese Funktion für Server-Routen). Nutzer ohne
// hire_date (Alt-Datensätze vor Einführung des Feldes) gelten bewusst als
// aktiv, sonst würden bestehende Mitarbeiter beim Rollout rückwirkend
// ausgegraut/gesperrt. atDate default = heute.
function isUserActive(user, atDate = new Date()) {
  if (!user) return false;
  const norm = d => !d ? null : (d instanceof Date ? d.toISOString().slice(0,10) : String(d).slice(0,10));
  const at = norm(atDate);
  const hire = norm(user.hire_date);
  const term = norm(user.termination_date);
  if (hire && at < hire) return false;
  if (term && at > term) return false;
  return true;
}
...
// Mitarbeiter werden flexibel auf Tag- ODER Nachtdienst eingeteilt (nicht fix
// einer Schichtart zugeordnet) — Kapazität, Score UND Vorschlag laufen daher
// einheitlich pro KALENDERTAG (5 Werte, CHRISTMAS_CALENDAR_DAY_KEYS), nicht
// mehr getrennt nach Tag-/Nachtdienst. Die konkrete Zuteilung zu Tag- oder
// Nachtdienst unter den "Arbeit vorgeschlagen"-Mitarbeitern erfolgt weiterhin
// im bestehenden Dienstplan-Modul, unter Berücksichtigung von Wunsch TD/ND
// als reiner Präferenz-Hinweis (kein Einfluss mehr auf diesen Vorschlag hier).
...
// Die 5 reinen Kalendertag-Keys — Domäne der Historien-Erfassung (eine Spalte
// je Tag, Schichtart steckt im Zellwert TD/ND) UND jetzt auch der Kapazitäts-/
// Vorschlagsberechnung.
const CHRISTMAS_CALENDAR_DAY_KEYS = CHRISTMAS_CALENDAR_DAYS.map(d => d.dateKey);

function christmasDateFor(year, calDayKey) {
  const day = CHRISTMAS_CALENDAR_DAYS.find(d => d.dateKey === calDayKey);
  if (!day) return null;
  const y = year + day.yearOffset;
  return `${y}-${String(day.month).padStart(2,'0')}-${String(day.day).padStart(2,'0')}`;
}

// Konfigurierbare Score-Gewichtung: NUR Nachtdienst (ND) an 24.12. und 31.12.
// zählt mit einem Zuschlag (Belastung höher als an den übrigen Tagen);
// Tagdienst ist nie gewichtet. Bewusst als benannte, zentrale Konstante statt
// verstreuter Magic Number — bei Bedarf hier (bzw. später über eine
// Einstellung) anpassbar.
const CHRISTMAS_SCORE_WEIGHT = 1.5;
const CHRISTMAS_NIGHT_WEIGHTED_DAYS = ['24.12', '31.12'];
// dayKey: 5-Key (ohne Schichtart-Suffix). status: nur für ND relevant.
function christmasScoreWeight(dayKey, status) {
  return status === 'ND' && CHRISTMAS_NIGHT_WEIGHTED_DAYS.includes(dayKey) ? CHRISTMAS_SCORE_WEIGHT : 1;
}

// Kumulativer Fairness-Score je Mitarbeiter × Kalendertag, über BELIEBIG viele
// Jahre (keine Beschränkung auf ein Lookback-Fenster). EINE Score-Dimension
// je Tag (24.12/25.12/26.12/31.12/01.01) — KEINE separate Tag-/Nacht-
// Dimension mehr; die Historie wird ebenfalls in genau einer Zelle je Tag
// erfasst (Schichtart steckt im Zellwert):
//   TD (Tagdienst gearbeitet)   → +1
//   ND (Nachtdienst gearbeitet) → +1, an 24.12./31.12. mit Faktor 1,5 (höhere Belastung)
//   UR (Urlaub)                 → −1
//   "–" (regulär frei)          →  0, trägt nicht zum Score bei
// Jahre/Tage ohne Eintrag tragen ebenfalls nichts bei — kein Sonderfall nötig.
// historyRows: [{employee_id,year,day_key,status}] mit day_key ∈
// CHRISTMAS_CALENDAR_DAY_KEYS (5 Werte) und status ∈ {TD,ND,UR,'–',''}.
// Rückgabe: {employeeId: {dayKey(5): score}}.
function computeChristmasScores(historyRows) {
  const scores = {};
  for (const h of historyRows) {
    if (!h.status) continue;
    let delta;
    if (h.status === 'TD') delta = 1;
    else if (h.status === 'ND') delta = christmasScoreWeight(h.day_key, 'ND');
    else if (h.status === 'UR') delta = -1;
    else continue; // '–' oder unbekannt → 0, kein Eintrag nötig
...
// employees: [{id,name}] — MUSS bereits auf Rotations-Teilnehmer UND aktive
//   Mitarbeiter gefiltert sein (ausgenommene/inaktive MA fließen weder in
//   Score noch Kapazität noch Vorschlag ein; sie werden separat im UI
//   angezeigt, aber hier nicht übergeben).
// historyRows: [{employee_id,year,day_key,status}] (ALLE Jahre).
// wishRows: [{employee_id,day_key,wish}] (nur aktuelles Jahr), wish ∈ {'U','TD','ND'}.
//   day_key ist der 5er-Kalendertag-Key — EIN Wunsch je Tag.
// shiftTypes: dp_shift_types-Zeilen (is_night unterscheidet Tag/Nacht)
// requirements: dp_shift_requirements-Zeilen
//
// Priorität pro Tag (NICHT mehr getrennt nach Schichtart — ein Mitarbeiter
// kann an einem Tag nicht gleichzeitig für Tagdienst "Arbeit" und für
// Nachtdienst "Urlaub empfohlen" sein, da es derselbe Kalendertag derselben
// Person ist):
//  1. Frei-Slots = Gesamt-MA − Gesamtbedarf (Tagdienst-Bedarf + Nachtdienst-
//     Bedarf zusammen, aus dem bestehenden Schichtbedarf-Modul).
//  2. MA mit Wunsch U nach Score ABSTEIGEND sortieren.
//  3. Die obersten X (X = Frei-Slots) erhalten "Urlaub empfohlen" — für den
//     gesamten Tag, ohne Aussage zur Schichtart.
//  4. Alle übrigen MA erhalten "Arbeit vorgeschlagen" — ebenfalls nur für den
//     Tag; die konkrete Zuteilung zu Tag- oder Nachtdienst unter diesen
//     Mitarbeitern erfolgt weiterhin im Dienstplan-Modul, unter
//     Berücksichtigung von Wunsch TD/ND als Präferenz-Hinweis (Wunsch TD/ND
//     hat hier KEINEN Einfluss mehr auf "Urlaub empfohlen" vs. "Arbeit
//     vorgeschlagen"). Innerhalb dieser Gruppe wird zusätzlich TAGE-
//     ÜBERGREIFEND ausgeglichen (CHRISTMAS_BALANCE_WEIGHT): wer an einem
//     früheren der 5 Tage bereits priorisiert wurde, rutscht an folgenden
//     Tagen zurück, damit sich die Arbeitstage auf mehrere Mitarbeiter
//     verteilen statt sich auf wenige Personen mit durchgehend niedrigem
//     Score zu konzentrieren.
//  5. Bei Score-Gleichstand entscheidet alphabetisch der Name (transparent,
//     nachvollziehbar — echtes Dienstalter bräuchte ein bislang nicht
//     vorhandenes "Mitarbeiter seit"-Feld).
//
// Zwei Nutzungsszenarien für den Wunsch am aktuellen Tag:
//  · Wunsch 'U'  = unverbindlicher Vorschlag/Wunsch, konkurriert wie oben
//    über den Score um die verbleibenden Frei-Slots ("wer ist am ehesten
//    berechtigt?").
//  · Wunsch 'UR' = bereits GENEHMIGTER, fixer Urlaub (z.B. weil der Planer
//    das schon entschieden hat) — wird IMMER als frei geführt, unabhängig
//    vom Score, und zieht zuerst von den Frei-Slots ab, bevor die
//    Wunsch-U-Kandidaten um die restlichen Slots konkurrieren. Übersteigt
//    die Anzahl genehmigter Urlaube die Frei-Slots, wird das als
//    capacityShortfall ausgewiesen (Unterbesetzung), OHNE die Genehmigung
//    rückgängig zu machen — der Planer muss das dann anderweitig lösen.
//
// Urlaubskontingent (aus dem bestehenden Modul "Dienstplan → Urlaubsübersicht",
// Tabelle vacation_config): begrenzt zusätzlich zur Personal-Kapazität (s.o.),
// wie viele MA an einem Tag GLEICHZEITIG Urlaub haben dürfen — unabhängig
// davon, wie viel Personal rein rechnerisch übrig wäre. Beide Grenzen gelten
// gemeinsam (die jeweils engere entscheidet). Ist für einen Tag kein
// Kontingent hinterlegt, gilt der portalweite Standard (CHRISTMAS_DEFAULT_VACATION_QUOTA),
// identisch zum Default in der Urlaubsübersicht selbst.
const CHRISTMAS_DEFAULT_VACATION_QUOTA = 8;
// Gewicht pro bereits (in diesem Vorschlag, an einem früheren Tag)
// priorisiertem Arbeitseinsatz — verhindert, dass dieselben Mitarbeiter
// (die an mehreren Tagen zufällig den niedrigsten Score haben) für ALLE
// diese Tage vorgeschlagen werden. Deutlich größer als übliche
// Score-Differenzen, damit "schon mal dran gewesen" beim Ausgleich
// zwischen den 5 Tagen spürbar stärker wiegt als kleine Score-Unterschiede.
const CHRISTMAS_BALANCE_WEIGHT = 3;

function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows, vacationQuota}) {
  const scores = computeChristmasScores(historyRows);
  const scoreOf = (empId, calDayKey) => scores[empId]?.[calDayKey] ?? 0;
  const wishOf = (empId, calDayKey) => (wishRows||[]).find(w => w.employee_id===empId && w.day_key===calDayKey)?.wish || '';
  const byScoreDesc = (a,b) => b.score - a.score || a.name.localeCompare(b.name,'de');
  // employeeId -> Anzahl Tage (in diesem Aufruf), an denen die Person bereits
  // zu den "requiredCount" dringendsten Arbeitskandidaten gehörte — wird
  // TAGWEISE IN REIHENFOLGE (24.12 → 01.01) aktualisiert, daher absichtlich
  // außerhalb der map()-Iteration deklariert (fortlaufender Zustand).
  const assignedDaysCount = {};

  return CHRISTMAS_CALENDAR_DAYS.map(calDay => {
    const date = christmasDateFor(year, calDay.dateKey);
    const holidays = getAustrianHolidays(year + calDay.yearOffset);
    const weekday = new Date(date).getDay();
    const dayInfo = {date, weekday, isWeekend: weekday===0||weekday===6, isHoliday: !!holidays[date]};

    // Gesamtbedarf = Tagdienst-Bedarf + Nachtdienst-Bedarf desselben Tages.
    const dayRequired = shiftTypes.filter(st => !st.is_night)
      .reduce((sum, st) => sum + getRequiredCount(requirements, st.id, dayInfo), 0);
    const nightRequired = shiftTypes.filter(st => !!st.is_night)
      .reduce((sum, st) => sum + getRequiredCount(requirements, st.id, dayInfo), 0);
    const requiredCount = dayRequired + nightRequired;
    const totalEmployees = employees.length;
    const freeSlots = Math.max(0, totalEmployees - requiredCount);

    const withScore = employees.map(e => ({
      ...e, score: scoreOf(e.id, calDay.dateKey), wish: wishOf(e.id, calDay.dateKey),
    }));

    // Urlaubskontingent für diesen Tag (separat von der Personal-Kapazität).
    const quotaMax = vacationQuota && Object.prototype.hasOwnProperty.call(vacationQuota, date)
      ? vacationQuota[date] : CHRISTMAS_DEFAULT_VACATION_QUOTA;

    // Genehmigter (fixer) Urlaub geht immer, unabhängig vom Score, und wird
    // zuerst von den Frei-Slots UND vom Urlaubskontingent abgezogen.
    const approvedIds = new Set(withScore.filter(e => e.wish === 'UR').map(e => e.id));
    // Die engere der beiden unabhängigen Grenzen (Personal-Kapazität vs.
    // Urlaubskontingent) bestimmt, wie viele MA insgesamt frei bekommen können.
    const effectiveOffCap = Math.min(freeSlots, quotaMax);
    const remainingFreeSlots = Math.max(0, effectiveOffCap - approvedIds.size);
    // Reicht die Personal-Kapazität nicht (mehr Genehmigungen als Frei-Slots),
    // fehlen Mitarbeiter für den Schichtbedarf.
    const capacityShortfall = Math.max(0, requiredCount - (totalEmployees - approvedIds.size));
    // Übersteigen allein schon die genehmigten Urlaube das Kontingent, ist das
    // unabhängig von der Personal-Kapazität ein Kontingent-Problem — in
    // beiden Fällen wird NICHTS automatisch storniert, nur ausgewiesen.
    const quotaShortfall = Math.max(0, approvedIds.size - quotaMax);

    // Wunsch U (nur unter den NICHT bereits genehmigten MA), nach Score
    // absteigend — die obersten verbleibenden Frei-Slots (nach beiden Grenzen)
    // bekommen "Urlaub empfohlen". Wunsch TD/ND ist hier nur Anzeige/
    // Präferenz, ohne Einfluss auf diese Entscheidung.
    const uWishers = withScore.filter(e => e.wish === 'U' && !approvedIds.has(e.id)).sort(byScoreDesc);
    const offRecommendedIds = new Set(uWishers.slice(0, remainingFreeSlots).map(e => e.id));

    // Arbeits-Pool: alle, die weder genehmigt noch (schon) als "Urlaub
    // empfohlen" gelten. Sortierung nach EFFEKTIVEM Score aufsteigend
    // (niedrigster zuerst = am ehesten dran) — der effektive Score erhöht
    // sich um CHRISTMAS_BALANCE_WEIGHT je Tag, an dem die Person an einem
    // FRÜHEREN Tag dieses Vorschlags bereits priorisiert wurde. Dadurch
    // verteilen sich die 5 Tage gleichmäßiger auf mehrere Mitarbeiter, statt
    // dieselben (durchgehend niedrigsten Score) Personen für alle Tage
    // vorzuschlagen, auf denen sie zufällig ganz vorne liegen.
    const workPool = withScore.filter(e => !approvedIds.has(e.id) && !offRecommendedIds.has(e.id));
    const effScore = e => e.score + (assignedDaysCount[e.id]||0) * CHRISTMAS_BALANCE_WEIGHT;
    const sortedWorkPool = [...workPool].sort((a,b) => effScore(a)-effScore(b) || a.name.localeCompare(b.name,'de'));
    // Die requiredCount dringendsten Kandidaten dieses Tages gelten als
    // "heute priorisiert" und werden dadurch an folgenden Tagen entlastet.
    sortedWorkPool.slice(0, requiredCount).forEach(e => {
      assignedDaysCount[e.id] = (assignedDaysCount[e.id]||0) + 1;
    });

    // Anzeige-Reihenfolge: "Arbeit vorgeschlagen" zuerst (das braucht eine
    // Entscheidung), dann "Urlaub empfohlen", ganz zuletzt "Urlaub genehmigt"
    // (das ist bereits fix entschieden und muss nicht mehr geprüft werden).
    const employeeRows = [
      ...sortedWorkPool.map(e => ({id: e.id, name: e.name, score: e.score, wish: e.wish, recommendation: 'work_suggested'})),
      ...withScore.filter(e => offRecommendedIds.has(e.id)).sort(byScoreDesc)
        .map(e => ({id: e.id, name: e.name, score: e.score, wish: e.wish, recommendation: 'off_recommended'})),
      ...withScore.filter(e => approvedIds.has(e.id)).sort(byScoreDesc)
        .map(e => ({id: e.id, name: e.name, score: e.score, wish: e.wish, recommendation: 'off_approved'})),
    ];

    return {
      dayKey: calDay.dateKey, label: calDay.dateLabel, dateKey: calDay.dateKey,
      date, weekday, isWeekend: dayInfo.isWeekend, isHoliday: dayInfo.isHoliday,
      requiredCount, dayRequired, nightRequired, totalEmployees, freeSlots,
      approvedCount: approvedIds.size, capacityShortfall,
      quotaMax, quotaShortfall,
      wishCount: uWishers.length,
...
module.exports = {
  ...
  isUserActive,
  // Weihnachtsdienst-Rotation
  CHRISTMAS_CALENDAR_DAYS, CHRISTMAS_CALENDAR_DAY_KEYS, christmasDateFor, computeChristmasScores, buildChristmasProposal,
  CHRISTMAS_SCORE_WEIGHT, CHRISTMAS_NIGHT_WEIGHTED_DAYS, christmasScoreWeight, CHRISTMAS_BALANCE_WEIGHT,
  CHRISTMAS_DEFAULT_VACATION_QUOTA,
  // Harte Regeln
