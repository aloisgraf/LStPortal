...
const CHRISTMAS_DAYS = [
  {key: '24.12', label: '24. Dezember (Heiliger Abend)', month: 12, day: 24, yearOffset: 0},
  {key: '25.12', label: '25. Dezember (Christtag)',       month: 12, day: 25, yearOffset: 0},
  {key: '26.12', label: '26. Dezember (Stefanitag)',      month: 12, day: 26, yearOffset: 0},
  {key: '31.12', label: '31. Dezember (Silvester)',       month: 12, day: 31, yearOffset: 0},
  {key: '01.01', label: '1. Jänner (Neujahr)',             month: 1,  day: 1,  yearOffset: 1},
];
...
// Kumulativer Fairness-Score je Mitarbeiter × Tag-Typ, über BELIEBIG viele Jahre
// (keine Beschränkung auf ein Lookback-Fenster): +1 pro Jahr gearbeitet (A),
// −1 pro Jahr Urlaub/frei (U). Jahre ohne Eintrag tragen nichts bei (kein
// Sonderfall nötig — ein Mitarbeiter, der an einem Tag laut regulärem
// Dienstplan ohnehin nie eingeteilt wäre, bekommt für den dort schlicht nie
// einen Historieneintrag und bleibt bei 0).
...
function computeChristmasScores(historyRows) {
  ...
  for (const h of historyRows) {
    ...
    const delta = h.status === 'A' ? 1 : h.status === 'U' ? -1 : 0;
...
// employees: [{id,name}]  historyRows: [{employee_id,year,day_key,status}] (ALLE Jahre)
// wishRows: [{employee_id,day_key,wants_off}] (nur aktuelles Jahr)
// shiftTypes: dp_shift_types-Zeilen  requirements: dp_shift_requirements-Zeilen
//
// Ablauf pro Tag: Frei-Slots = Gesamt-MA − Schichtbedarf. Von den MA mit
// Urlaubswunsch werden die mit dem NIEDRIGSTEN Score (= am längsten nicht frei
// gehabt) bis zur Slot-Anzahl als "Urlaub empfohlen" vorgeschlagen. Bei
// Score-Gleichstand entscheidet alphabetisch der Name (transparent,
// nachvollziehbar — echtes Dienstalter bräuchte ein bislang nicht vorhandenes
// "Mitarbeiter seit"-Feld). Alle übrigen MA gelten als "Arbeit vorgeschlagen".
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  return CHRISTMAS_DAYS.map(day => {
    ...
    const requiredCount = shiftTypes.reduce(
    ...
    // Wünschende zuerst nach Score (aufsteigend), dann alphabetisch sortieren —
    ...
    return {
      dayKey: day.key, label: day.label, date, weekday,
      isWeekend: dayInfo.isWeekend, isHoliday: dayInfo.isHoliday,
