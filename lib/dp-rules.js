...
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  return CHRISTMAS_CALENDAR_DAYS.map(calDay => {
    ...
    // Anzeige-Reihenfolge: erst nach Empfehlungsgruppe (Genehmigt → Empfohlen
    // → Arbeit vorgeschlagen), innerhalb der Off-Gruppen nach Score
    // ABSTEIGEND (höchster Score = hat am meisten gearbeitet = am meisten
    // verdient). Innerhalb "Arbeit vorgeschlagen" dagegen nach Score
    // AUFSTEIGEND (niedrigster Score = hat bisher am wenigsten gearbeitet =
    // ist am ehesten dran) — sonst stünden ausgerechnet die Mitarbeiter mit
    // dem höchsten "hat schon oft gearbeitet"-Score ganz oben in der
    // Arbeits-Liste, obwohl sie fairnessmäßig eher Urlaub verdient hätten.
    // Das greift auch, solange noch niemand einen Wunsch eingetragen hat.
    const recOrder = {off_approved: 0, off_recommended: 1, work_suggested: 2};
    const employeeRows = withScore
      .map(e => ({
        id: e.id, name: e.name, score: e.score, wish: e.wish,
        recommendation: approvedIds.has(e.id) ? 'off_approved'
          : offRecommendedIds.has(e.id) ? 'off_recommended'
          : 'work_suggested',
      }))
      .sort((a,b) => {
        const byRec = recOrder[a.recommendation] - recOrder[b.recommendation];
        if (byRec !== 0) return byRec;
        const dir = a.recommendation === 'work_suggested' ? 1 : -1;
        return dir * (a.score - b.score) || a.name.localeCompare(b.name,'de');
      });
...
module.exports = {
  ...
  CHRISTMAS_SCORE_WEIGHT, CHRISTMAS_NIGHT_WEIGHTED_DAYS, christmasScoreWeight,
  // Harte Regeln
