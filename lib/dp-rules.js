...
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  return CHRISTMAS_CALENDAR_DAYS.map(calDay => {
    ...
    // Wunsch U, nach Score absteigend — die obersten Frei-Slots bekommen
    // "Urlaub empfohlen". Wunsch TD/ND ist hier nur Anzeige/Präferenz, ohne
    // Einfluss auf diese Entscheidung.
    const uWishers = withScore.filter(e => e.wish === 'U').sort(byScoreDesc);
