...
// employees: [{id,name}]  historyRows: [{employee_id,year,day_key,status}]
// shiftTypes: dp_shift_types-Zeilen  requirements: dp_shift_requirements-Zeilen
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows}) {
  return CHRISTMAS_DAYS.map(day => {
    ...
    // Verpflichtet = hatte in den letzten N Jahren an GENAU DIESEM Tag (day_key)
    // Urlaub — jeder Tag wird einzeln betrachtet, kein Gesamtblock.
    const obligated = employees.filter(emp =>
      historyRows.some(h => h.employee_id===emp.id && h.day_key===day.key && h.status==='U')
    );

    const gap = requiredCount - obligated.length;
    return {
      dayKey: day.key, label: day.label, date, weekday,
      isWeekend: dayInfo.isWeekend, isHoliday: dayInfo.isHoliday,
      requiredCount,
      obligated,
      obligatedCount: obligated.length,
      coverage: gap > 0 ? 'under' : gap < 0 ? 'over' : 'ok',
      gap,
    };
  });
}
...
module.exports = {
  ...
  getRequiredCount,
  // Weihnachtsdienst-Rotation
  CHRISTMAS_DAYS, CHRISTMAS_DAY_KEYS, christmasDateFor, buildChristmasProposal,
  // Harte Regeln
