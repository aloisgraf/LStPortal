...
// Der Schichtbedarf (Kapazität) wird weiterhin separat für Tag-/Nachtdienst
// berechnet (CHRISTMAS_DAYS/CHRISTMAS_DAY_KEYS, 10 Kombinationen, dayKey z.B.
// "24.12-T"/"24.12-N") — der Fairness-SCORE dagegen kennt nur noch EINE
// Dimension je Kalendertag (CHRISTMAS_CALENDAR_DAY_KEYS, 5 Werte), da Historie
// und Wunsch pro Tag in einer einzigen Zelle erfasst werden.
...
// Die 5 reinen Kalendertag-Keys (ohne Schichtart-Suffix) — Domäne der
// Historien-Erfassung (eine Spalte je Tag, Schichtart steckt im Zellwert TD/ND).
...
//   day_key ist der 5er-Kalendertag-Key — EIN Wunsch je Tag, nicht je Schichtart.
// shiftTypes: dp_shift_types-Zeilen (is_night unterscheidet Tag/Nacht)
// requirements: dp_shift_requirements-Zeilen
//
// Priorität pro Tag/Schichtart (Bedarf weiterhin separat für Tag- und
// Nachtdienst, da der Schichtbedarf das vorgibt — der Score dahinter ist aber
// für beide Schichtarten am selben Tag identisch):
//  1. Frei-Slots = Gesamt-MA − Schichtbedarf.
//  2. Freiwillige (Wunsch TD/ND passend zur Schichtart) decken den Bedarf
//     zuerst ab — bei mehr Freiwilligen als Plätzen entscheidet der
//     NIEDRIGSTE Score (hat am wenigsten gearbeitet, "arbeitet am ehesten
//     freiwillig ohnehin schon fällig").
//  3. Reicht das nicht, werden die restlichen Plätze score-basiert
//     aufsteigend (niedrigster Score zuerst) aus den übrigen MA aufgefüllt.
//  4. Die übrigen (nicht eingeteilten) MA mit Wunsch U erhalten, nach Score
//     ABSTEIGEND sortiert, den Vorschlag "Urlaub empfohlen" — begrenzt durch
//     die Frei-Slots. MA ohne Wunsch U, die ebenfalls nicht gebraucht werden,
//     gelten als "regulär frei" (kein aktiver Urlaubswunsch).
...
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  const byNameScore = (a,b) => a.score - b.score || a.name.localeCompare(b.name,'de');
  const byScoreDesc = (a,b) => b.score - a.score || a.name.localeCompare(b.name,'de');

  return CHRISTMAS_CALENDAR_DAYS.flatMap(calDay => {
    const holidays = getAustrianHolidays(year + calDay.yearOffset);

    return CHRISTMAS_SHIFT_KINDS.map(sk => {
      const dayKey = `${calDay.dateKey}-${sk.suffix}`;
      const date = christmasDateFor(year, dayKey);
      const weekday = new Date(date).getDay();
      const dayInfo = {date, weekday, isWeekend: weekday===0||weekday===6, isHoliday: !!holidays[date]};

      const relevantShiftTypes = shiftTypes.filter(st => !!st.is_night === (sk.kind === 'night'));
      ...
      // Schritt 2: Freiwillige (Wunsch passend zur Schichtart) zuerst — bei
      // Überangebot arbeiten die mit dem niedrigsten Score (fair: sie "sind
      // dran"), die übrigen Freiwilligen rutschen in den Rest-Pool (Schritt 4).
      const wishKind = sk.kind === 'day' ? 'TD' : 'ND';
      const volunteers = withScore.filter(e => e.wish === wishKind).sort(byNameScore);
      const workerIds = new Set(volunteers.slice(0, requiredCount).map(e => e.id));

      // Schritt 3: fehlende Plätze score-basiert aufsteigend auffüllen
      if (workerIds.size < requiredCount) {
        const need = requiredCount - workerIds.size;
        withScore
          .filter(e => !workerIds.has(e.id))
          .sort(byNameScore)
          .slice(0, need)
          .forEach(e => workerIds.add(e.id));
      }

      // Schritt 4: Rest-Pool — nur MA mit Wunsch U werden aktiv als "Urlaub
      // empfohlen" markiert, nach Score absteigend, begrenzt durch Frei-Slots.
      const remaining = withScore.filter(e => !workerIds.has(e.id));
      const uWishers = remaining.filter(e => e.wish === 'U').sort(byScoreDesc);
      const offRecommendedIds = new Set(uWishers.slice(0, freeSlots).map(e => e.id));

      const employeeRows = withScore
        .map(e => ({
          id: e.id, name: e.name, score: e.score, wish: e.wish,
          recommendation: workerIds.has(e.id) ? 'work_suggested'
            : offRecommendedIds.has(e.id) ? 'off_recommended'
            : 'off_available', // regulär frei, kein aktiver Urlaubswunsch
        }))
        .sort(byScoreDesc);

      return {
        dayKey, label: `${calDay.dateLabel} – ${sk.label}`, dateKey: calDay.dateKey, shiftKind: sk.kind,
        date, weekday, isWeekend: dayInfo.isWeekend, isHoliday: dayInfo.isHoliday,
        requiredCount, totalEmployees, freeSlots,
        volunteerCount: volunteers.length,
        wishCount: uWishers.length + volunteers.length,
...
module.exports = {
  ...
  isUserActive,
  // Weihnachtsdienst-Rotation
  CHRISTMAS_DAYS, CHRISTMAS_DAY_KEYS, CHRISTMAS_CALENDAR_DAYS, CHRISTMAS_CALENDAR_DAY_KEYS, christmasDateFor, computeChristmasScores, buildChristmasProposal,
