...
function buildChristmasProposal(year, {employees, shiftTypes, requirements, historyRows, wishRows}) {
  ...
  return CHRISTMAS_CALENDAR_DAYS.map(calDay => {
    ...
    // zuerst von den Frei-Slots abgezogen.
    const approvedIds = new Set(withScore.filter(e => e.wish === 'UR').map(e => e.id));
    const remainingFreeSlots = Math.max(0, freeSlots - approvedIds.size);
    // Reicht die Kapazität nicht (mehr Genehmigungen als Frei-Slots), fehlen
    // Mitarbeiter für den Schichtbedarf — wird ausgewiesen, aber die bereits
    // genehmigten Urlaube werden NICHT automatisch storniert.
    ...
    // absteigend — die obersten verbleibenden Frei-Slots bekommen "Urlaub
    // empfohlen". Wunsch TD/ND ist hier nur Anzeige/Präferenz, ohne Einfluss
