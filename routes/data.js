...
router.get('/', auth, async (req,res) => {
  try {
    ...
    const itemMap={};
    (itemsRaw||[]).forEach(it=>{if(!itemMap[it.instance_id])itemMap[it.instance_id]=[];itemMap[it.instance_id].push({id:it.id,instanceId:it.instance_id,title:it.title,description:it.description,status:it.status,dueDate:it.due_date,meetingDate:it.meeting_date,parentId:it.parent_id,delegatedTo:it.delegated_to,result:it.result,sortOrder:it.sort_order,createdBy:it.created_by,createdAt:it.created_at,updatedAt:it.updated_at||it.created_at,groupId:it.group_id||null,link:it.link||null,convertedTicketId:it.converted_ticket_id||null,convertedAt:it.converted_at||null,convertedBy:it.converted_by||null,protokoll:(()=>{try{return JSON.parse(it.protokoll||'[]');}catch{return [];}})(),participants:partMap[it.id]||[],externalParticipants:(()=>{try{return JSON.parse(it.external_participants||'[]');}catch{return [];}})(),aiStatus:it.ai_status||null,aiResult:(()=>{try{return it.ai_result?JSON.parse(it.ai_result):null;}catch{return null;}})(),docLinks:docLinksByTarget[it.id]||[],contactLinks:contactLinksByTarget[it.id]||[],priority:it.priority||'medium'});});
    const protoMap={};
    (protocolsRaw||[]).forEach(pr=>{if(!protoMap[pr.instance_id])protoMap[pr.instance_id]=[];protoMap[pr.instance_id].push({id:pr.id,instanceId:pr.instance_id,title:pr.title,date:pr.date,time:pr.time||'',location:pr.location||'',attendees:(()=>{try{return JSON.parse(pr.attendees||'[]');}catch{return [];}})(),body:pr.body||'',released:!!pr.released,type:pr.type||'protocol',createdBy:pr.created_by,createdAt:pr.created_at,updatedAt:pr.updated_at,docLinks:docLinksByTarget[pr.id]||[],contactLinks:contactLinksByTarget[pr.id]||[]});});
    const apptMap={};
    (appointmentsRaw||[]).forEach(a=>{if(!apptMap[a.instance_id])apptMap[a.instance_id]=[];apptMap[a.instance_id].push({id:a.id,instanceId:a.instance_id,title:a.title,date:a.date,time:a.time||'',location:a.location||'',createdBy:a.created_by,createdAt:a.created_at,updatedAt:a.updated_at});});
    const instMap={};
    (instancesRaw||[]).forEach(inst=>{if(!instMap[inst.meeting_id])instMap[inst.meeting_id]=[];instMap[inst.meeting_id].push({id:inst.id,meetingId:inst.meeting_id,date:inst.date,time:inst.time||'',title:inst.title||null,status:inst.status,notes:inst.notes||'',kind:inst.kind||'points',createdBy:inst.created_by,createdAt:inst.created_at,items:itemMap[inst.id]||[],protocols:protoMap[inst.id]||[],files:instFileMap[inst.id]||[],appointments:apptMap[inst.id]||[]});});
