...
router.post('/docs', auth, async (req, res) => {
  ...
  try {
    ...
    const id = newId();
    const ext = extFromMime(mimeType || 'application/octet-stream', originalName);
    ...
    const doc = await q1(
      ...
      [id, categoryId||null, title, description||'', filename, originalName, mime, buf.length, req.uid]
    );
    ...
    await q(
      ...
      [vId, id, filename, originalName, buf.length, req.uid]
    );
    ok(res, doc);
  } catch(e) { bad(res, e.message, 500); }
});
...
router.post('/docs/:id/version', auth, async (req, res) => {
  try {
    ...
    fs.writeFileSync(path.join(DOCS_DIR, newFilename), buf);
    // Archive old current version record (already inserted at upload); update documents table
    const vId = newId();
    await q(
      `INSERT INTO document_versions (id,document_id,version,filename,original_name,size_bytes,uploaded_by)
    ...
    // Update documents to new version
    const updated = await q1(
      ...
      [newFilename, originalName, mime, buf.length, newVersion, doc.id]
    );
    ok(res, updated);
  } catch(e) { bad(res, e.message, 500); }
});
...
router.get('/docs/:id/versions/:vid', auth, async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename="${encodeURIComponent(ver.original_name)}"`);
    res.sendFile(filePath);
  } catch(e) { bad(res, e.message, 500); }
});
